...
int create_multiple_vluns_216(struct ctx *p_ctx)
{
    int rc,j;
    ...
    for (j=0; j < 10; j++)
    {
        ...
        {
            rc = create_vluns_216(fldisks[0].dev,
...
int ioctl_7_1_216( int flag )
{
    int rc,i;
    struct ctx myctx, myctx1,myctx2,myctx3;
    struct ctx *p_ctx = &myctx;
    struct ctx *p_ctx1 = &myctx1, *temp=&myctx2, *temp1=&myctx3;
    pthread_t thread;
    ...
    memset(temp1, 0, sizeof(struct ctx));
    ...
    // need to do following in a loop...
    for (i=1;i<long_run; i++ )
    {
...
#endif
        // initiating fd to second disk.
        if ( i/2 == 0 )
            p_ctx->fd=temp->fd;              // for every even iteration, use fd of first disk
        else
            p_ctx->fd=temp1->fd;            // for every odd iteration, use fd of second disk

        rc=ioctl_dk_capi_attach(p_ctx);  // doing detach
        CHECK_RC(rc, "Context with REUSE failed");

        // if reuse succeeded. detach the context for other disk, reverse the fd for detaching
        p_ctx->flags=0;
        if ( i/2 == 0 )
            p_ctx->fd=temp1->fd;
        else
        ...
        /* spawn the VLUNs */
        // now re initiating again
        if ( i/2 == 0 )
            p_ctx->fd=temp->fd;              // for every even iteration, use fd of first disk
        else
            p_ctx->fd=temp1->fd;            // for every odd iteration, use fd of second disk

        rc=create_multiple_vluns_216(p_ctx);
        close_res(p_ctx);
    }
