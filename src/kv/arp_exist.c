...
void ark_exist_start(_ARK *_arkp, int tid, tcb_t *tcbp)
{
  ...
  if (tcbp->hblk == 0)
  {
    KV_TRC(pAT, "ENOENT  tid:%3d ttag:%3d key:%p klen:%ld pos:%ld",
           tid, tcbp->ttag, rcbp->key, rcbp->klen, rcbp->posI);
    rcbp->res   = -1;
    rcbp->rc    = ENOENT;
  ...
  {
      KV_TRC(pAT, "RE_INB  tid:%3d ttag:%3d old:%ld new:%ld",
  ...
  KV_TRC(pAT, "RD_HASH tid:%3d ttag:%3d", tid, tcbp->ttag);
...
void ark_exist_finish(_ARK *_arkp, int tid, tcb_t *tcbp)
{
  ...
  KV_TRC(pAT, "INB_GET tid:%3d ttag:%3d tot:%ld used:%ld",
               tid, tcbp->ttag, tcbp->inb_size, (uint64_t)tcbp->inb->len);

  // Find the key position in the read in bucket
  rcbp->res = bt_exists(tcbp->inb, rcbp->klen, rcbp->key);
  if (rcbp->res == BT_FAIL)
  {
    KV_TRC(pAT, "ENOENT  tid:%3d ttag:%3d pos:%ld key:%p klen:%ld",
