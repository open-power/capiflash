...
/*                                                                        */
/* $Source: src/kv/test/fvt_ark_io_inject.C $                             */
/*                                                                        */
 ...
 * \file
 * \brief
 *   inject random r/w/alloc errors while running multi-context
 * \ingroup
 ******************************************************************************/
 ...
 ******************************************************************************/
TEST(FVT_KV_ARK_IO, ASYNC_CB_ARK_IO_INJECT)
{
    uint32_t ctxts       = 100;
    uint32_t ops         = 20;
    uint32_t vlen        = KV_4K;
    ...
    if (!env_FVT_DEV) ctxts=10;

    kv_async_init_ark_io_inject(ctxts, ops, vlen, xmin*60);
    kv_async_start_jobs();
